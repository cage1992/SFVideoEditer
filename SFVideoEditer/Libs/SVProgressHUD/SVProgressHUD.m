...
static const CGFloat SVProgressHUDRingRadius = 10; //18
static const CGFloat SVProgressHUDRingNoTextRadius = 12; //24
...
- (void)setStatus:(NSString *)string updatePosition:(BOOL)updatePos;
...
- (void)updatePosition {
    ...
    CGFloat stringHeightBuffer = 12;
    CGFloat stringAndImageHeightBuffer = 60;
    ...
    if(string) {
        ...
        stringWidth = stringRect.size.width < 150 ? 150 : stringRect.size.width; //最低宽度150
        stringHeight = ceil(stringRect.size.height) < 30 ? 30 : ceil(stringRect.size.height); // 最低高度30
        ...
        CGFloat labelRectY = imageUsed ? 50 : 9;
    ...
    NSInteger imageCenterY = 28;

    if(string)
        self.imageView.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, imageCenterY);
	else
	...
	if(string) {
        ...
        CGPoint center = CGPointMake((CGRectGetWidth(self.hudView.bounds)/2), imageCenterY);
        self.indefiniteAnimatedView.center = center;
        
        if(self.progress != -1)
            self.backgroundRingLayer.position = self.ringLayer.position = CGPointMake((CGRectGetWidth(self.hudView.bounds)/2), imageCenterY);
	}
    else {
